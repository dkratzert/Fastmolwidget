...
- `viewer_widget.py` / `viewer_widget3D.py` — `MoleculeViewerWidget` / `MoleculeViewer3DWidget` bundle a renderer + two-row control bar. Both expose `.render_widget` and `.load_file(path)`.
  - **Row 1** (both): Grow, Pack Unit Cell, Show ADP, Show Labels, Hide Hydrogens (`checked` → `show_hydrogens(False)`). Grow and Pack are mutually exclusive (toggling one unchecks the other).
  - **Row 2** (both): Bond Width spinbox (2D: 1–15, 3D: 0–15), Bond Color button (opens `QColorDialog`)
  - **Row 2 extra** (3D only): Reset Rotation Center button → calls `render_widget.reset_rotation_center()`
