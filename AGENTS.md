...
- `viewer_widget.py` / `viewer_widget3D.py` — `MoleculeViewerWidget` / `MoleculeViewer3DWidget` bundle a renderer + control bar (Grow, Show ADP, Show Labels, Show Hydrogens, Bond Width 1–15). Both viewers expose `.render_widget` and `.load_file(path)`.
