...
- `hkl_io.py` — Qt-free readers for reflection data and SHELX refinement parameters: `read_shelx_hkl` (fixed-format `HKLF 4`, free-format fallback, stops at the `0 0 0` terminator), `read_cif_reflections` (fcf-style `_refln_*` loops, reuses `F_calc` when present), and `read_shelx_parameters` → `ShelxParameters(osf, wght_a, wght_b, exti, wavelength, free_variables)`. Parameters are looked up in the order: the `.res`/`.ins` itself → a sibling `.res`/`.ins` of the same basename → a SHELX block embedded in the CIF (`embedded_shelx_res`, tags `_shelx_res_file` / `_iucr_refine_instructions_details`).
- `density.py` — residual (Fo−Fc) maps, **Qt-free**. `calculate_residual_density(model, hkl)` → `ResidualDensityMap` (`.array` numpy grid over one unit cell, `.max/.min/.rms/.d_min/.scale`, `.isosurface(level, atoms=…)` → Cartesian `(vertices, edges)` for `GL_LINES`). Uses gemmi for the SF summation and FFT. Three non-obvious requirements when building a `gemmi.SmallStructure` by hand — all three silently give wrong results if missed: (1) `site.element` must be set (`type_symbol` is only a label, otherwise everything scatters as `X`); (2) `structure.setup_cell_images()` must be called (the SF calculator reads symmetry from `UnitCell`, not from the space group); (3) CIF-read structures need `change_occupancies_to_crystallographic()`. Map coefficients follow SHELXL's own convention: unweighted `(|Fo|/OSF − |Fc|)·exp(iφc)` — `WGHT` is *not* applied to Fourier coefficients.
  - **Temporary workaround**: `_shelx_occupancies()` decodes SHELX SOF codes from `atom.sof` instead of using `atom.occupancy`, because *shelxfile* (v24) mis-decodes negative SOFs with a site-occupancy factor ≠ 1 (`Atom._get_negative_occupancy` returns `1 + fvar*occ` instead of `|occ|*(1-fvar)`) and lets an `AFIX` card without an explicit SOF overwrite a riding atom's own SOF with `11.0`. Remove once shelxfile is fixed. SHELX's negative-`U_iso` riding convention (`-1.5` = 1.5 × U_eq of the pivot) is also resolved there.
- `cpp/density_cpp` (`src/density_cpp/density_cpp.cpp`) — optional pybind11 marching-cubes extension, built by `setup.py` alongside `sdm_cpp`. `marching_cubes(grid, level, origin, step) -> (vertices, edges)` with deduplicated wireframe edges. Guarded by `density.HAS_DENSITY_CPP`; there is **no** pure-Python fallback — the feature raises a clear `RuntimeError` and the viewer disables its button instead.
