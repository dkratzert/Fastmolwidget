...
- **Public API additions** must be reflected in: the relevant widget class, `MoleculeWidgetProtocol`, `__init__.py` `__all__`, and `README.md`. Current protocol methods: `open_molecule`, `clear`, `show_adps`, `show_labels`, `show_hydrogens`, `set_visible_parts`, `set_bond_width`, `set_bond_color`, `set_labels_visible` (alias for `show_labels`), `set_background_color`, `setLabelFont`, `reset_view`, `align_best_view`, `save_image`. `Atomtuple`, `MoleculeQuickItem`, `MoleculeViewerBackend`, and `MoleculeRendererMixin` are exported in `__all__`.
